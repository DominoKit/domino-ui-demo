...
package org.dominokit.domino.datatable.client.views.gridtable.plugins.pagination;

import elemental2.core.JsMath;
import elemental2.dom.HTMLTableElement;
import org.dominokit.domino.ui.elements.DivElement;
import org.dominokit.domino.datatable.client.views.gridtable.DataTable;
import org.dominokit.domino.datatable.client.views.gridtable.events.BodyScrollEvent;
...
public class BodyScrollPlugin<T>
  ...
  public void onBodyAdded(DataTable<T> dataTable) {
    DivElement scrollElement = dataTable.tableElement();
    scrollElement.addEventListener(
        "scroll",
        evt -> {
          double scrollTop = new Double(scrollElement.element().scrollTop).intValue();
          if (scrollTop == 0) {
            dataTable.fireTableEvent(new BodyScrollEvent(ScrollPosition.TOP));
          }
          int offsetHeight = new Double(scrollElement.element().offsetHeight).intValue();
          int scrollHeight = new Double(scrollElement.element().scrollHeight).intValue();
          int clientHeight = new Double(scrollElement.element().clientHeight).intValue();
