...
import org.dominokit.domino.datatable.client.views.gridtable.plugins.DataTablePlugin;
import org.dominokit.domino.datatable.client.views.gridtable.plugins.column.ResizeColumnMeta;
...
public class TableConfig<T>
  ...
  private String gridColumnsTemplate="";
  private String gridRowsTemplate="";
   ...
   */
  public void drawHeaders(DataTable<T> dataTable, DivElement thead) {
    ...
    DivElement[] headers = new DivElement[maxDepth + 1];
    for (int i = 0; i < headers.length; i++) {
      headers[i] = div().addCss(dui_datatable_row);
    ...
    columns.forEach(col -> {
      ...
      dataTable.setCssProperty(widthProperty);
    ...
    updateGridColumnsTemplate();
    dataTable.bodyElement().setCssProperty(CssProperty.of(GRID_TEMPLATE_COLUMNS, getGridColumnsTemplate()));
    headers[0].setCssProperty(CssProperty.of(GRID_TEMPLATE_COLUMNS, getGridColumnsTemplate()));
    thead.nowAndWhenAttached(() -> {
      DomGlobal.requestAnimationFrame(p-> {
          columns.forEach(col -> {
            CssProperty widthProperty = col.getFixedWidthProperty();
            dataTable.setCssProperty(widthProperty);
          });
      });
    });
  ...
  public void updateGridColumnsTemplate(){
    StringBuilder headersTemplate = new StringBuilder();
    StringBuilder rowsTemplate = new StringBuilder();
    columns.forEach(col -> {
      CssProperty widthProperty = col.getWidthProperty();
      CssProperty widthFixedProperty = col.getFixedWidthProperty();
      dataTable.setCssProperty(widthProperty);
      dataTable.setCssProperty(widthFixedProperty);
      headersTemplate.append("var(");
      headersTemplate.append(widthProperty.getName());
      headersTemplate.append(") ");
      rowsTemplate.append("var(");
      rowsTemplate.append(widthFixedProperty.getName());
      rowsTemplate.append(") ");
    });
    gridColumnsTemplate =  headersTemplate.toString();
    gridRowsTemplate =  rowsTemplate.toString();
  }

  public String getGridColumnsTemplate() {
    return gridColumnsTemplate;
  }

  public String getGridRowColumnsTemplate() {
    return gridRowsTemplate;
  }
