...
import org.dominokit.domino.datatable.client.views.gridtable.events.*;
import org.dominokit.domino.datatable.client.views.gridtable.model.SearchContext;
import org.dominokit.domino.datatable.client.views.gridtable.store.DataStore;
...
public class DataTable<T> extends BaseDominoElement<HTMLDivElement, DataTable<T>>
  ...
  private DivElement tableElement;
  private TableConfig<T> tableConfig;
  private DivElement tbody = div().addCss(dui_datatable_body);
  private DivElement thead = div().addCss(dui_datatable_thead);
  private DivElement tfoot = div().addCss(dui_datatable_tfoot);
  ...
  public DataTable(TableConfig<T> tableConfig, DataStore<T> dataStore) {
    tableElement = div().addCss(dui_datatable, dui_datatable_width_full);
    super.init(this);
    ...
    bodyElement().onResize((element1, observer, entries) -> {
      this.tableConfig.updateGridColumnsTemplate();
    });
  }
  ...
  private void addRows(List<T> data, int initialIndex) {
    this.bodyElement().appendChild(form = form().addCss(dui_fixed,
            dui_border_1,
            dui_border_solid,
            dui_border_black,
            dui_invisible));
    ...
    for (int index = 0; index < data.size(); index++) {
      ...
      tableRow.setCssProperty(TableConfig.GRID_TEMPLATE_COLUMNS, tableRow.getDataTable().getTableConfig().getGridRowColumnsTemplate());
   ...
   */
  public DivElement tableElement() {
    return tableElement;
  }
   ...
   */
  public DivElement headerElement() {
    return thead;
  }
   ...
   */
  public DivElement footerElement() {
    return tfoot;
  }
   ...
   * @return the current DataTable instance
   */
  public DataTable<T> withTable(ChildHandler<DataTable<T>, DivElement> handler) {
   ...
   * @return the current DataTable instance
   */
  public DataTable<T> withTableBody(ChildHandler<DataTable<T>, DivElement> handler) {
   ...
   * @return the current DataTable instance
   */
  public DataTable<T> withTableFooter(ChildHandler<DataTable<T>, DivElement> handler) {
   ...
   * @return the current DataTable instance
   */
  public DataTable<T> withTableHead(ChildHandler<DataTable<T>, DivElement> handler) {
