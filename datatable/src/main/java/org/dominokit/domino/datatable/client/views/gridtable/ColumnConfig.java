...
import org.dominokit.domino.datatable.client.views.grid.GridUtil;
...
import static org.dominokit.domino.datatable.client.views.gridtable.ColumnUtils.fixElementWidth;
...
public class ColumnConfig<T> implements ElementsFactory, DataTableStyles {
  ...
  public ColumnConfig(String name, String title) {
    this.name = name;
    this.title = title;
    this.uniqueId = DominoId.unique("col-"+name +"-");
   ...
   */
  public DominoElement<HTMLDivElement> getHeadElement() {
   ...
   */
  public void applyScreenMedia(IsElement<? extends Element> element) {
    applyScreenMedia(element.element());
  }
  /**
   * Applies the screen media styles to the provided element.
   *
   ...
   */
  void renderHeader(DataTable<T> dataTable, TableConfig<T> tableConfig, DivElement[] headers) {
  ...
  public CssProperty getFixedWidthProperty() {
    return CssProperty.of("--dui-table-" + uniqueId + "-width-fixed", Unit.px.of(getFixedWidth()));
  }

  private double getFixedWidth() {
    DOMRect rect = this.headElement.element().getBoundingClientRect();
    return rect.width;
  }
